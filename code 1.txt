...
if mode == "Generate Image":
    ...
    image_prompt = st.text_area(
        ...
        "Generate a logo of 'APTECH' with a black background. The word 'APTECH Learning' should have a modern, sleek font, with the color of 'APTECH' in red and white. The overall design should be clean, professional, and convey a sense of innovation and technology."
    )
    if st.button("Generate Image"):
        if image_prompt:
            with st.spinner("Generating image..."):
                try:
                    # Original model name. If it fails, check the "List Models" output.
                    image_model_name = "gemini-2.0-flash-preview-image-generation"
                    
                    # You might need to change this based on List Models output
                    # Example if a new name is found: image_model_name = "gemini-1.5-flash-latest-image"
